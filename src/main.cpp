  ...
  while (!WindowShouldClose()) {
    ...
    if (ImGui::TreeNode("Station")) {
      ...
      if (ImGui::TreeNode("Story")) {
        ...
        for (int i = 0; i < 32; i++) {
          if (ImGui::Button(
                  TextFormat("%d/%d##BUTTON_%d", i, state.story.flags[i], i))) {
      ...
      if (ImGui::TreeNode("Animation/Sounds")) {
        ...
        ImGui::Text("Current Walk Cycle: %d", state.station.anim.currentCycle);
    ...
    if (ImGui::TreeNode("Ship")) {
      ...
      ImGui::Text("Ship Rot: (rad/real) %.02f \n\t\t  (deg) %.02f",
                  state.ship.shipRotation, state.ship.shipRotation * RAD2DEG);
