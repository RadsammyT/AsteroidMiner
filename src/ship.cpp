...
void DoShip(GameState *state, Camera3D *cam) {
  ...
  if (state->debug.ship2dRep) {
    ...
    DrawLineV(
        ...
        Vector2Add(state->ship.shipPosition,
                   Vector2Scale(state->ship.shipVelocity, 1 / GetFrameTime())),
        WHITE);
  ...
  for (auto i : state->ship.asteroids) {
    ...
    DrawCircle(100 + (x * 80), 100 + (y * 80), (float)i.size * 1.5,
               dist <= 2.5 + i.getSphereRad() ? RED : BLACK);
  }
  ...
  DrawTexturePro(state->textures.shipLaserGauge, {0, 0, 32, 64},
                 {WIDTH - (200 - 32), HEIGHT - 300, 64 * 2, 128 * 2}, {0, 0}, 0,
                 WHITE);
  ...
  if (state->transition.active) {
    if (state->transition.onPeak == STATION_TO_SHIP) {
      // fade-out at descend
      // the below commented declaration would make fade-out work on desktop,
      // but not for web, as we are going to 0..-255. Casting to u8 (unsigned
      // char) on desktop gets us 255..0 but on web, casting to u8 gets us
      // constant 0s, which is why web transitions bugs out.
...
void ShipAsteroid::DrawAst(GameState *state, Camera3D *cam) {
  ...
  if (SphereInFrustumV(&frus, {pos.x, 0, pos.y}, radius)) {
    ...
    if (lasered) {
      temp =
          Vector3Add(temp, {RAND_FLOAT / 10, RAND_FLOAT / 10, RAND_FLOAT / 10});
    }
    DrawModelEx(
        *model, temp,
        {cosAsteroidRotation.x, cosAsteroidRotation.y, cosAsteroidRotation.z},
        GetTime() * 10, Vector3Scale(Vector3One(), 2.5f / divisor), WHITE);
    if (monster) {
      DrawBillboardRec(*cam, state->textures.shipMonsterSprite, {0, 0, 32, 32},
                       {pos.x, 0, pos.y}, {5, 5}, {128, 128, 128, 255});
    }
#if DEBUG_MODE
    ...
    float dist = sqrt(abs(distvec.x * distvec.x) + abs(distvec.y + distvec.y));
    DrawText(TextFormat("size: %d, dura: %.01f, dist: %0.2f, b:%d, m:%d", size,
                        durability, dist, burrowed, monster),
